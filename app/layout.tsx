...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const HERO = process.env.NEXT_PUBLIC_APP_HERO_IMAGE!;
  const SPLASH = process.env.NEXT_PUBLIC_APP_SPLASH_IMAGE!; // synced with .env
  const SPLASH_BG =
    process.env.NEXT_PUBLIC_SPLASH_BACKGROUND_COLOR || "#0A0A0A";

  const absolute = (u?: string) =>
    u && u.startsWith("http") ? u : `${URL.replace(/\/$/, "")}${u || ""}`;
